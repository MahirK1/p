...
export default function OrderManagerChatPage() {
  ...
  useEffect(() => {
    if (showCreateModal && commercials.length === 0) {
      ...
      fetch("/api/users?role=COMMERCIAL")
        ...
        .then((data) => {
          // Provjeri da li je data array
          if (Array.isArray(data)) {
            setCommercials(data);
          } else {
            console.error("API did not return an array:", data);
            setCommercials([]);
            showToast("Greška: Neispravan format podataka", "error");
          }
        ...
        `}>
          {selectedRoomId ? (
            ...
            <div className="flex flex-1 flex-col items-center justify-center text-center p-8 bg-slate-50/20">
              ...
              <h3 className="text-xl font-bold text-slate-900">Order Manager Chat</h3>
              <p className="text-slate-500 max-w-xs mt-2 text-sm leading-relaxed">
                Odaberite razgovor s lijeve strane da biste započeli komunikaciju sa komercijalistima.
              </p>
            </div>
          )}
        </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {loadingCommercials ? (
                ...
                commercials.map((commercial) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-slate-900 truncate">{commercial.name || "Nepoznato"}</p>
                ...
                <div className="flex flex-col items-center justify-center p-12 text-center">
                  <p className="text-slate-500 text-sm">Nema dostupnih komercijalista</p>
                </div>
              )}
            </div>
          </div>
