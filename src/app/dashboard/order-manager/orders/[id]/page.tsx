...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadOrder = async () => {
    ...
    if (data.status === "PENDING" && !statusChanged) {
      await fetch("/api/orders", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderId: data.id, status: "APPROVED" }),
      });
      setStatusChanged(true);
      const updatedRes = await fetch(`/api/orders?id=${id}`, { cache: "no-store" });
      if (updatedRes.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="invoice-container rounded-2xl border border-slate-200 bg-white shadow-sm print:border-none print:shadow-none print:rounded-none">
        {/* Minimal header - kompaktniji za print */}
        ...
        {/* Informacije u grid formatu - kompaktnije za print */}
        ...
        <div className="px-8 py-6 print:px-2 print:py-2">
          ...
          {/* Napomena komercijaliste - kompaktnija za print */}
          {order.note && (
